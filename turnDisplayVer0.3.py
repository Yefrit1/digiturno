...
class Digiturno(QMainWindow):
    ...
    def positionOverlayBox(self):
        self.turnWarning.move(
            (self.width() - self.turnWarning.width()) // 2,
            (self.height() - self.turnWarning.height()) // 2
        )

    """def nextTurn(self, ticket):
        self.turnWarning.setText(f"Turno: {self.turno}")
        self.animation_step = 0
        self.timer.start(800)"""

    def updateAnimation(self):
        if self.animation_step < 10:
            self.turnWarning.setVisible(self.animation_step % 2 == 0)
            self.animation_step += 1
        else:
            self.timer.stop()
            self.turnWarning.hide()
